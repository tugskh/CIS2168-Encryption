...
public class CircularLinkedList<E> implements Iterable<E> {
	...
	private Node<E> getNode(int index ) {
        ...
        public E setNode(int index, E item){
            if(index<0||index>=size){
                throw new IndexOutOfBoundsException(":(");
            }
	...
	public E remove(int index) {
        ...
        if(size == 1){ // removing the only item
            toReturn = head.item;
            head = null;
            tail = null;
        }else if(index == 0){ //removing the first item
            toReturn = head.item;
            head = head.next;
            tail.next = head;
        } else if(index == size -1){ // removing the last item
            toReturn = tail.item;
            Node<E> before = getNode(index - 1);
            tail = before;
            tail.next = head;
